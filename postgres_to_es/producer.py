...
from models import ETLEnricherData, ETLProducerTable
from decorator import coroutine, some_sleep
from postgres import ETLPG
...
class ETLProducer:
    ...
    producer_table = [
        ...
        ETLProducerTable(table='genre', field='film_work_id', ptable='genre_film_work', pfield='genre_id'),
    ]
    ...
    def producer(self, enricher):
        ...
        while True:
            ...
            try:
                lasttime = self.redis.set_lasttime(data.table, idlist[-1].updated_at)
            except IndexError:
